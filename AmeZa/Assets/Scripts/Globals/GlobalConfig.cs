...
public class GlobalConfig : StaticConfig<GlobalConfig>
{
    ...
    public string ConfigUrl => string.Empty;// $"http://seganx.com/games/ameza/{version}/config.txt?{System.DateTime.Now.Ticks}";
    public string JokesUrl => string.Empty;//$"http://seganx.com/games/ameza/jokes.txt?{System.DateTime.Now.Ticks}";
    public string ApiUrl => string.Empty;//$"http://seganx.com/games/ameza/api-v1";
    ...
    private void OnValidate()
    {
        ...
        {
            item.sku = item.sku.Replace("ameza_", "").Replace("a_", "");
            item.header = $"{item.sku,-10}\t{item.price,-10:N0}\t{item.discount,-2}\t{item.StorePriceLabel,-4}";
        }
    }
    ...
    public static Data.Shop Shop => Instance.data.shop;
    ...
    public class Data
    {
        ...
        {
            [System.Serializable]
            public class AdPlace : Plankton.Ad.Place
            {
                public int timerId = 50;
                public int interval = 1800;
                public int rewardGems = 100;
                public AdPlace(Plankton.Ad.Type adType, string placeName) : base(adType, placeName) { }
        ...
        {
            [System.Serializable]
            public class PricedPackage
            {
                [HideInInspector] public string header = string.Empty;
                public string sku = string.Empty;
                public int price = 0;
                public int discount = 0;

                public string StorePriceLabel { get; set; } = string.Empty;
                public string StoreLastPriceLabel { get; set; } = string.Empty;
                public string StoreDiscountLabel { get; set; } = string.Empty;
            }

            [System.Serializable]
            public class Package : PricedPackage
            {
            ...
            [System.Serializable]
            public class VIP : PricedPackage
            {
                public int days = 7;
            }
            ...
            public List<PricedPackage> GetAll()
            {
                var result = new List<PricedPackage>();
                result.AddRange(vips);
                result.AddRange(offers);
                result.AddRange(combinedPackages);
                result.AddRange(gemPackages);
                return result;
            }
        }
